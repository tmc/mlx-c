...
import (
	...
	"github.com/tmc/mlx-c/tools/mlx-c-gen/internal/mlxcgen/hooks"
	"github.com/tmc/mlx-c/tools/mlx-c-gen/internal/mlxcgen/inventory"
...
func main() {
	...
	case "-h", "-help", "--help":
		...
		return
	default:
		fmt.Fprintf(os.Stderr, "mlx-c-gen: unknown command %q\n\n", os.Args[1])
		usage()
		os.Exit(2)
	}
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "mlx-c-gen %s: %v\n", os.Args[1], err)
		os.Exit(1)
	}
}
...
func usage() {
  ...
  mlx-c-gen generate <root> [flags]   regenerate from <root>'s codegen inputs
  mlx-c-gen check <root> [flags]      verify outputs against <root>'s inputs
  mlx-c-gen parse <root> [flags]      debug: dump normalized IR
...
everything that drives generation.
  ...
  <root>/codegen/    manifest.yaml, custom/, types.yaml, compat-waivers.yaml,
                     removals.yaml, mlxc-capi.lock.json

Outputs are a separate, explicit destination:

  mlx-c-gen generate . --out ~/ml-explore/mlx-c/mlx/c --mlx-src ~/mlx

When --out is omitted, generate writes to <root>/mlx/c. check verifies
...
// the first token after the verb: "mlx-c-gen <verb> <root> [flags]".
func takeRoot(args []string) (string, []string, error) {
	if len(args) == 0 || strings.HasPrefix(args[0], "-") {
		return "", nil, fmt.Errorf("missing <root>; usage: mlx-c-gen <verb> <root> [flags]")
	}
	return args[0], args[1:], nil
}

func runGenerate(args []string) error {
	fs := flag.NewFlagSet("mlx-c-gen generate", flag.ContinueOnError)
	fs.SetOutput(os.Stderr)
	mlxSrc := fs.String("mlx-src", "", "Path to MLX source directory")
	compileCommandsPath := fs.String("compile-commands", "", "compile_commands.json path for parser flags")
	reportPath := fs.String("report", "", "Path to output generation report JSON file")
	typesOut := fs.String("types-out", "", "Write type-policy YAML to path (override-only; nothing is written without it)")
	...
	noFormat := fs.Bool("no-format", false, "Skip running clang-format on generated files")
	...
	customDir := filepath.Join(root, "codegen", "custom")

	mlxSrcPath, err := resolveMLXSource(root, *mlxSrc)
	if err != nil {
		return err
	}
	...
	parser.SetCompileCommandsPath(compileCommands)
	...
	// Generate type-policy metadata if requested
	if *typesOut != "" && success {
...
func runParse(args []string) error {
	...
	customSpecs, err := loadCustomSpecs(opts.RepoRoot, opts.CustomDir, opts.Jaccl)
	if err != nil {
		return err
	}
...
func parseOptionsFromArgs(args []string) (parseOptions, error) {
	...
	fs.SetOutput(os.Stderr)
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "Usage: mlx-c-gen parse <root> [options]")
	...
	fs.BoolVar(&opts.Jaccl, "jaccl", false, "include opt-in custom bindings (jaccl)")
	...
	mlxSrcResolved, err := resolveMLXSource(opts.RepoRoot, opts.MLXSrc)
	if err != nil {
		return opts, err
	}
	opts.MLXSrc = mlxSrcResolved
	opts.ManifestPath = filepath.Join(opts.RepoRoot, "codegen", "manifest.yaml")
	opts.CustomDir = filepath.Join(opts.RepoRoot, "codegen", "custom")
...
func checkReportCNameUniqueness(opts checkOptions, report *regenreport.Report) error {
	...
	customSpecs, err := loadCustomSpecs(opts.Options.RepoRoot, opts.Options.CustomDir, opts.Options.Jaccl)
	if err != nil {
		return err
	}
...
func loadCustomSpecs(root, dir string, includeOptIn bool) ([]customspec.Spec, error) {
	if dir == "" {
		return nil, nil
	}
	...
	return customspecsForRun(specs, includeOptIn), nil
}

// customspecsForRun drops opt-in specs unless explicitly requested.
func customspecsForRun(specs []customspec.Spec, includeOptIn bool) []customspec.Spec {
	if includeOptIn {
		return specs
	}
	out := specs[:0:0]
...
func runCheck(args []string) error {
	...
	lock, err := checkAPILock(opts, report)
	if err != nil {
		return err
	}
	...
	if err := checkUpstreamParity(opts, lock, manifest); err != nil {
		return err
	}
	if err := checkLockMonotonicity(opts, lock, manifest); err != nil {
		return err
	}
	return nil
}
...
type checkOptions struct {
	...
	ParityWaiverPath  string
	RemovalWaiverPath string

	// Baseline is a monotonicity baseline: an existing lock file path or a git
	// ref. The default is HEAD, meaning the committed lock.
	Baseline string
}
...
func parseCheckOptions(args []string) (checkOptions, error) {
	...
	fs.SetOutput(os.Stderr)
	fs.Usage = func() {
		fmt.Fprintln(os.Stderr, "Usage: mlx-c-gen check <root> [options]")
		fmt.Fprintln(os.Stderr)
		fs.PrintDefaults()
	}
	var symbolTargets targetFlags
	compileCommandsPath := fs.String("compile-commands", "", "compile_commands.json path for parser flags (never derived for check)")
	outputs := fs.String("outputs", "", "tree root whose generated files are verified (default <root>; may point into another checkout)")
	jaccl := fs.Bool("jaccl", false, "Include opt-in custom bindings (jaccl) in verification")
	...
	upstreamHeaders := fs.String("upstream-headers", "", "upstream mlx-c header directory for the shared-symbol parity gate")
	baseline := fs.String("baseline", "HEAD", "monotonicity baseline: git ref or lock file path")
	...
	// All inputs derive from <root>/codegen/. There are no path overrides:
	// a tree is verified against its own lineage or not at all.
	opts.LockPath = filepath.Join(root, "codegen", "mlxc-capi.lock.json")
	opts.LockTUPath = filepath.Join(root, "codegen", "lock.c")
	opts.ParityWaiverPath = filepath.Join(root, "codegen", "compat-waivers.yaml")
	opts.RemovalWaiverPath = filepath.Join(root, "codegen", "removals.yaml")
	opts.Baseline = *baseline

	mlxSrcResolved, err := resolveMLXSource(root, *mlxSrc)
	if err != nil {
		return opts, err
	}
	opts.Options = regenreport.Options{
		...
		ManifestPath:        filepath.Join(root, "codegen", "manifest.yaml"),
		CustomDir:           filepath.Join(root, "codegen", "custom"),
		TypePolicyPath:      filepath.Join(root, "codegen", "types.yaml"),
	...
	opts.UpstreamHeaders = *upstreamHeaders
	return opts, nil
}
...
func checkAPILock(opts checkOptions, report *regenreport.Report) (*apilock.Lock, error) {
	if opts.LockPath == "" {
		if report != nil && report.Manifest.Report.RequireAPILock {
			return nil, fmt.Errorf("manifest requires API lock but no lock path is configured")
		}
		return nil, nil
	}
...
// resolveMLXSource resolves the MLX C++ source directory. The flag wins;
// otherwise <root>/codegen/mlx-src.path — data in the tree, not discovery —
// supplies it; otherwise this is an error naming both ways.
func resolveMLXSource(root, explicit string) (string, error) {
	if explicit != "" {
		return explicit, nil
	}
	pathFile := filepath.Join(root, "codegen", "mlx-src.path")
	if data, err := os.ReadFile(pathFile); err == nil {
		if line := strings.TrimSpace(string(data)); line != "" {
			return line, nil
		}
	}
	return "", fmt.Errorf("no MLX source: pass --mlx-src PATH or write the directory to %s", pathFile)
}

// deriveCompileCommands implements the tool's only derivation rule: for
// generation, an unset --compile-commands next to an MLX source checked out as
// _deps/mlx-src finds the sibling compile_commands.json. Every derived use is
// logged. An explicit path that does not exist is a hard error. Check never
// derives: verification does not guess inputs.
func deriveCompileCommands(mlxSrc, explicit string) (string, error) {
	if explicit != "" {
		if _, err := os.Stat(explicit); err != nil {
			return "", fmt.Errorf("compile commands %s: %w", explicit, err)
		}
		return explicit, nil
	}
	if filepath.Base(mlxSrc) == "mlx-src" && filepath.Base(filepath.Dir(mlxSrc)) == "_deps" {
		candidate := filepath.Join(filepath.Dir(mlxSrc), "compile_commands.json")
		if _, err := os.Stat(candidate); err == nil {
			fmt.Fprintf(os.Stderr, "using compile_commands: %s (derived)\n", candidate)
...
func init() {
	// The dispatcher in main prints its own usage for the bare and unknown
	// cases; nothing here uses the global flag.CommandLine parse path.
}
